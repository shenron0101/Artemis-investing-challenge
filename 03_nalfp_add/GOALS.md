# Stage 03 — NALFP Additions: Goals & Design Brief

**Status:** planning (no implementation yet)
**Date:** 2026-05-29
**Predecessor:** NALFP v3 (commit `66bea36`)

> **Run order.** This stage starts with the network producer
> `00_network_dynamics.py` (reads the stage-02 characteristics panel and writes
> `network_panel.parquet`, the source of the network factors), then runs the
> universe → returns → fundamentals → factor-validation scripts. See the
> top-level `README.md` for the full command sequence.
...
1. **Regime-conditional tilt (recommended).** Scale the net-long exposure by the regime proxy
   already in the panel — the **stablecoin-inflow z-score** (`02_artemis_econometrics/01_build_panel.py`).
...
Built in `03_nalfp_add/`: `01_universe_coverage.py` (Binance price audit) →
...
the NALFP v3 definitions (size, momentum, low-vol, NetMom, NetRel via networkx
