#!/usr/bin/env python3
"""Stage 15 - factor sub-book ensemble strategy.

This stage uses the Stage 14 factor panel and XGBoost regime probabilities, but
does not blend every factor into one monolithic score. It builds three sub-books
and allocates among them with a causal rolling performance rule:
...
STAGE14 = ROOT / "14_regime_factor_strategy"
DATA14 = STAGE14 / "artifacts" / "data"
...
def load_stage14():
    module_path = STAGE14 / "run.py"
    spec = importlib.util.spec_from_file_location("stage14_run_for_stage15", module_path)
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame]:
    missing = [
        path
        for path in [DATA14 / "factor_panel.parquet", DATA14 / "regime_panel.parquet"]
        if not path.exists()
    ]
    if missing:
        raise FileNotFoundError(f"Run 14_regime_factor_strategy/run.py first; missing {missing}")
    panel = pd.read_parquet(DATA14 / "factor_panel.parquet")
    regime = pd.read_parquet(DATA14 / "regime_panel.parquet")
...
def factor_weight_frame(panel: pd.DataFrame, factor_weights: dict[str, float]) -> pd.DataFrame:
    ...
    row_weights = {fac: max(factor_weights.get(fac, 0.0), 0.0) / total for fac in s14.FACTOR_ORDER}
    fw = pd.DataFrame({"week": sorted(panel["week"].dropna().unique())})
    for fac in s14.FACTOR_ORDER:
...
def build_subbook(spec: BookSpec, panel: pd.DataFrame, regime: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    signal = s14.build_signal(panel, fw)
    candidate = s14.Candidate(
        name=spec.name,
        sleeve_core=1.0,
        sleeve_priced=0.0,
    ...
    weights = s14.construct_weights(signal, regime, candidate)
    pnl = s14.backtest(weights, panel)
    pnl["book"] = spec.name
...
def plot_cumulative(
    ...
    plt.yscale("log")
    plt.title("Stage 15 Factor Ensemble, Net Returns")
...
Train/test split matches Stage 14: weeks before `{first_oos:%Y-%m-%d}` are
in-sample, and the final {OOS_WEEKS} weeks are out-of-sample.
...
def main() -> None:
    ...
    print(f"Loaded Stage 14 panel: {len(panel):,} rows, {len(weeks):,} weeks", flush=True)
