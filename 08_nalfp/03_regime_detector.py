...
Two cross-sectional signals are available each week:
    ...
    E_gx_it    — Pillar 2 Giglio-Xiu factor-model expected return

The blended expected return is

    E_final_it = w_net_t * E_net_it + (1 - w_net_t) * E_gx_it
...
def load_signals() -> pd.DataFrame:
    """Stitch network + GX + forward returns into one wide panel."""
    net = pd.read_parquet(DATA_DIR / "network_panel.parquet")
    net["week"] = pd.to_datetime(net["week"])
    gx = pd.read_parquet(DATA_DIR / "gx_expected_returns.parquet")
    gx["week"] = pd.to_datetime(gx["week"])
    df = net.merge(gx, on=["week", "symbol"], how="left")
    df = df.rename(columns={"within_cluster_mom": "E_net"})
    return df.dropna(subset=["E_net", "E_gx", "fwd_ret_1w"]).reset_index(drop=True)
...
def rolling_weights(ic_net: pd.Series, ic_gx: pd.Series) -> pd.DataFrame:
    """w_net_t = clip+(IC_net_{t-1}) / [clip+(IC_net_{t-1}) + clip+(IC_gx_{t-1})]
    using a rolling 8-week mean of *past* ICs — strictly OOS.

    If both ICs are non-positive over the lookback, fall back to 0.5/0.5."""
    weeks = ic_net.index.union(ic_gx.index).sort_values()
    ic_net = ic_net.reindex(weeks)
    ic_gx = ic_gx.reindex(weeks)
    roll_net = ic_net.shift(1).rolling(IC_LOOKBACK, min_periods=3).mean()
    roll_gx = ic_gx.shift(1).rolling(IC_LOOKBACK, min_periods=3).mean()
    pos_net = roll_net.clip(lower=0.0)
    pos_gx = roll_gx.clip(lower=0.0)
    denom = pos_net + pos_gx
    w_net = np.where(denom > 0, pos_net / denom.replace(0, np.nan), 0.5)
    return pd.DataFrame({
        "week": weeks,
        "ic_net_roll": roll_net.values,
        "ic_gx_roll": roll_gx.values,
        "w_net": w_net,
    })
...
def blend(df: pd.DataFrame, weights: pd.DataFrame) -> pd.DataFrame:
    ...
    out["E_gx_z"] = out.groupby("week")["E_gx"].transform(cs_zscore)
    out["w_net"] = out["w_net"].fillna(0.5)
    out["E_final"] = out["w_net"] * out["E_net_z"] + (1.0 - out["w_net"]) * out["E_gx_z"]
    return out
...
def _figures(weights: pd.DataFrame) -> None:
    ...
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                        subplot_titles=("Rolling 8w IC — network vs GX",
    ...
    fig.add_trace(go.Scatter(x=weights["week"], y=weights["ic_gx_roll"],
                             mode="lines", name="IC_gx", line=dict(color="#d62728")), row=1, col=1)
    ...
    fig.update_layout(title="Pillar 3 — adaptive signal blend (Network vs Giglio-Xiu)",
...
def main() -> None:
    ...
    ic_gx = per_week_ic(df, "E_gx")
    weights = rolling_weights(ic_net, ic_gx)
    blended = blend(df, weights)

    write_frame(blended[["week", "symbol", "E_net_z", "E_gx_z", "w_net", "E_final",
    ...
    manifest = {
        ...
        "ic_gx_full_sample_mean": float(ic_gx.mean()),
        "ic_net_oos_mean": float(ic_net.iloc[TRAIN_WEEKS:].mean()),
        "ic_gx_oos_mean": float(ic_gx.iloc[TRAIN_WEEKS:].mean()),
