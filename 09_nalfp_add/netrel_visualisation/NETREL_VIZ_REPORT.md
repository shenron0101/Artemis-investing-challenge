...
The IC t-stat is **-1.72** in-sample (borderline, reversed) and
**-0.10** out-of-sample (not significant). The GX pricing test
...
The raw IC = -0.0280 IS (t = -1.72). This is slightly
...
cycle). OOS the IC is essentially zero (-0.0025, t = -0.10).
...
|---|---|---|
| Weeks | 184 | 79 |
| Annualised Return | 25.7% | 9.1% |
| Sharpe Ratio | 0.69 | 0.23 |
| Mean IC | -0.0280 | -0.0025 |
| IC t-stat (NW) | -1.72 (marginal) | -0.10 (not significant) |
| Return t-stat (NW) | +1.35 (not significant) | +0.31 (not significant) |

### Return Distribution

| Statistic | IS | OOS |
|---|---|---|
| Mean weekly return | 0.575% | 0.247% |
| Std (weekly) | 4.970% | 5.415% |
| Skewness | 0.47 | -0.28 |
| Excess Kurtosis | 3.43 | -0.04 |
...
- IS t = -1.72 — borderline/not significant (slight reversal direction)
- OOS t = -0.10 — not significant (near zero)
