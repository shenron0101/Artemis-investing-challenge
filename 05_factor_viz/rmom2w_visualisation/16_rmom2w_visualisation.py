"""09 — RMOM2w (2-Week Risk-Adjusted Momentum) Factor Visualisation Pipeline.

RMOM2w = 2-week return / 4-week rolling volatility.
A slightly slower-horizon risk-adjusted momentum signal from Han et al. (2023).

Grade: Suggestive — IS IC significant (t = -2.46, reversed direction),
OOS IC fades (t = -0.24), but ASSD-dominant over Bitcoin (ε₂ = 0.003).

Outputs
-------
    artifacts/figures/rmom2w_*.html / .png
    artifacts/data/rmom2w_viz_data.parquet
    RMOM2W_VIZ_REPORT.md
"""
...
FRAC = 0.30
MIN_NAMES = 10
T_LABELS = ["Low-RMOM2", "Mid-RMOM2", "High-RMOM2"]
SPREAD_LABEL = "RMOM2w (High–Low)"
...
GX_T = 1.05
ASD_EPS1 = 0.404
ASD_EPS2 = 0.003   # ASSD-dominant over Bitcoin
...
def build_characteristics(panel, trade_symbols):
    ...
    ret_2w = px_wide / px_wide.shift(2) - 1.0   # 2-week return
    rmom2w = ret_2w / vol4.replace(0, np.nan)    # 2-week return / 4-week vol

    # Also compute RMOM1w for comparison chart
    ...
    for df, nm in [(fwd, "fwd_ret_1w"), (rmom2w, "rmom_2w"), (rmom1w, "rmom_1w")]:
...
def build_rmom1w_ic(chars):
    keep = chars.dropna(subset=["fwd_ret_1w", "rmom_1w"])
    return keep.groupby("week").apply(
        lambda b: b["rmom_1w"].rank().corr(b["fwd_ret_1w"].rank())
        if len(b) >= MIN_NAMES else np.nan
    ).rename("ic_rmom1w")
...
def chart_tercile_returns(chars, is_lo, is_hi, oos_lo, oos_hi):
    ...
    fig.update_layout(template=PLOTLY_TEMPLATE, height=500, barmode="group",
                      ...
                      xaxis_title="RMOM2w Group", yaxis_title="Annualised Return (%)",
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig, spread
...
def chart_horizon_ic_comparison(ic_1w, ic_2w, is_lo, is_hi, oos_lo, oos_hi):
    """Chart 9: Rolling mean IC for RMOM1w vs RMOM2w — horizon comparison.
    Shows how the 1-week vs 2-week horizon changes IC stability and direction."""
    roll_1w = ic_1w.rolling(26).mean().dropna()
    roll_2w = ic_2w.rolling(26).mean().dropna()

    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08,
                        subplot_titles=("26-Week Rolling Mean IC: RMOM1w vs RMOM2w",
                                        "Difference: RMOM2w IC − RMOM1w IC"))
    fig.add_trace(go.Scatter(x=roll_1w.index, y=roll_1w.values,
                             name="RMOM1w IC", line=dict(color=COLORS["IS"], width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=roll_2w.index, y=roll_2w.values,
                             name="RMOM2w IC", line=dict(color=COLORS["OOS"], width=2, dash="dash")),
                  row=1, col=1)
    fig.add_hline(y=0, line_dash="dash", line_color="#666", row=1, col=1)

    # Difference (aligned index)
    combined = pd.concat([roll_1w.rename("r1"), roll_2w.rename("r2")], axis=1).dropna()
    diff = combined["r2"] - combined["r1"]
    fig.add_trace(go.Bar(x=diff.index, y=diff.values,
                         marker_color=COLORS["FULL"], marker_opacity=0.6,
                         name="RMOM2w − RMOM1w"), row=2, col=1)
    ...
    fig.update_layout(template=PLOTLY_TEMPLATE, height=700,
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    fig.update_yaxes(title_text="Rolling Mean IC", row=1, col=1)
    fig.update_yaxes(title_text="IC Difference", row=2, col=1)
    return fig
...
def write_report(row, is_d, oos_d, adf_stat, adf_p, jb_is, jb_is_p, jb_oos, jb_oos_p):
    md = f"""# RMOM2w (2-Week Risk-Adjusted Momentum) — Analysis Report
...
In-sample, the IC is **{fmt_val(row['IS_IC_t'])}** — significant, but in the *wrong* direction
(reversal: high-RMOM2w coins underperformed). Out-of-sample, the IC fades to
**{fmt_val(row['OOS_IC_t'])}** (near zero). However, the ASSD test confirms the return
distribution beats Bitcoin for risk-averse investors (ε₂ = {ASD_EPS2:.3f}).
...
The raw IC = **{fmt_val(row['IS_IC'], '.4f')}** IS, t = **{fmt_val(row['IS_IC_t'])}** — significant
...
Out-of-sample: IC = **{fmt_val(row['OOS_IC'], '.4f')}**, t = **{fmt_val(row['OOS_IC_t'])}** — the
...
| ASSD vs BTC | ε₂ = 0.000 ✓ | ε₂ = {ASD_EPS2:.3f} ✓ |
...
From the OOS IC near zero (t = {fmt_val(row['OOS_IC_t'])}), the overreaction pattern
...
Bitcoin for risk-averse investors (ε₂ = {ASD_EPS2:.3f} ≤ 0.032). The statistical
...
- IS t = {fmt_val(row['IS_IC_t'])} — |t| = {fmt_val(abs(row['IS_IC_t']))} — **significant (reversal)**
...
Full GX (K_hidden = 2): **λ = +{GX_LAMBDA:.1f}%/yr, t = +{GX_T:.2f}** — not significant (|t| < 1.65).
...
L/S return is positive IS ({fmt_val(row['IS_annret']*100, '.1f')}%) — driven by the short-the-winner
IS regime — and positive but modest OOS ({fmt_val(row['OOS_annret']*100, '.1f')}%).
...
Positive skew IS ({fmt_val(is_d['skew'], '.2f')}) — the L/S spread occasionally captures
...
IS Sharpe ({fmt_val(row['IS_sharpe'])}) is modest; OOS ({fmt_val(row['OOS_sharpe'])}) remains
...
![Horizon IC comparison](artifacts/figures/{PREFIX}_09_horizon_ic_comparison.png)
...
"""
    (STAGE / "RMOM2W_VIZ_REPORT.md").write_text(md)
    print("  wrote RMOM2W_VIZ_REPORT.md")
...
def main():
    ...
    fig9 = chart_horizon_ic_comparison(ic_1w, factor_ic, is_lo, is_hi, oos_lo, oos_hi)
    ...
    charts = [
        ...
        (f"{PREFIX}_09_horizon_ic_comparison", fig9),
    ]
