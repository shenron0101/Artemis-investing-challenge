...
TVL & Total value locked: a DeFi protocol engagement metric sourced from Artemis Terminal; strongly priced as a systematic risk but not a reliable standalone weekly trading signal & \ieeecitetwo{brigida-2025}{brigida-2026} \\
...
Crypto factor research has a structural advantage over equity factor research that shapes this taxonomy. Beyond price and volume, every protocol publishes its own balance sheet on-chain: fee revenue, total value locked, active addresses, transaction counts, and capital flows are all directly observable and update continuously. These on-chain metrics are the closest analogue crypto has to traditional fundamentals, but they are public from the first block, free of disclosure lag, and immune to managerial discretion in reporting. The strategy uses on-chain data in two places: FunC ranks protocols by fee-to-market-cap (the cash-flow yield), and TVLC ranks by TVL-to-market-cap (the engagement yield). Both signals are sourced from the Artemis Terminal API and treated economically as crypto-native fundamental factors. Crucially, the empirical results show that on-chain engagement signals tend to be \emph{priced} rather than \emph{tradable} at weekly frequency---a finding revisited throughout the report and exploited in the strategy's factor routing.
...
TVLC & Long high TVL-to-market-cap, short low & This captures DeFi engagement per dollar of market value, using TVL data from Artemis Terminal. The bull case is usage-backed value; the bear case is TVL irrelevance because the information is already priced. The sign is genuinely ambiguous before testing.\ieeecite{brigida-2025} \\
\bottomrule
\end{tabularx}
\end{center}

TVLC illustrates a pattern that recurs across the factor set: being strongly priced is not the same as being a good weekly trading signal. The Giglio--Xiu test assigns TVLC a large negative risk premium ($\lambda = -185.2\%/\text{yr}$, $|t| = 4.71$), meaning high-TVL/market-cap assets earn \emph{lower} expected returns — investors appear to pay a DeFi engagement premium, leaving low-TVL protocols underpriced on a risk-adjusted basis. Yet TVLC has no measurable weekly IC. The Artemis Terminal data confirms the usage signal is real; the market simply prices it in at a slow, structural pace rather than week to week. This is why TVLC is routed to the Priced Tilt sleeve rather than the weekly ranker book.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/network_clusters.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/netmom_netrel_schematic.png}
\caption{NetMom and NetRel illustrated on the same network. NetMom works inside one cluster by buying the local momentum leader and shorting the local laggard. NetRel works between clusters by buying coins in the leading narrative bloc and shorting coins in the lagging bloc.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/factor_routing_graph.png}
...
Most pairs are close to zero. The one meaningful correlation, NEWC with CRASH8 at $+0.63$, is economically sensible because younger coins are more crash-prone. Both still survive jointly in the GX model, so the pair is not simply one trade wearing two labels.

TVLC's role in the Priced Tilt sleeve is structurally different from the four behavioral factors and deserves separate emphasis. It is the only direct on-chain fundamental signal in the production strategy: each weekly TVL-to-market-cap ratio is computed from protocol balance-sheet data delivered through the Artemis Terminal API rather than from price returns. Conceptually it is the crypto analogue of an equity value factor where TVL stands in for book equity. The GX test assigns it the strongest non-market priced premium in the fundamental family ($\lambda = -185.2\%/\text{yr}$, $|t| = 4.71$), and the negative sign indicates that protocols with very high engagement-to-price ratios are bid up to expensive valuations and underperform on a risk-adjusted basis. Pairing a behavioral sleeve with an on-chain fundamental anchor is deliberate: the behavioral factors capture investor preferences over crash, lottery, beta, and seasoning, while TVLC captures the slow repricing of usage-backed protocol value. The two channels are economically independent, and the residual correlations confirm it---no behavioral factor has a meaningful correlation with TVLC in our sample.
...
\subsubsection*{Formal Allocation Algorithm}

Let $b \in \{\text{mispricing},\, \text{core\_rank},\, \text{priced\_tilt}\}$, $t$ denote the current week, and $\alpha_b$ the variant's fixed base allocation. The five-step procedure applied each week is:

\textbf{Step 1 — Rolling performance score.} Using the past 26 weeks of book returns:
\begin{equation*}
  s_b(t) = \max\!\bigl(\widehat{\mathrm{SR}}_{26}(b,t),\;0\bigr)\;\times\;\bigl(1 + \min\!\bigl(\widehat{\mathrm{MDD}}_{26}(b,t),\;0\bigr)\bigr)
\end{equation*}
where $\widehat{\mathrm{SR}}_{26}$ is the 26-week rolling Sharpe ratio and $\widehat{\mathrm{MDD}}_{26}$ is the 26-week rolling maximum drawdown. The score is zero if the book has been losing, and is penalised further by how deep it has drawn down.

\textbf{Step 2 — Blend with base.} Once at least 8 weeks of history are available:
\begin{equation*}
...
\textbf{Step 3 — Regime tilt.} Let $p_{\mathrm{on}}$, $p_{\mathrm{neutral}}$, $p_{\mathrm{off}}$ be the XGBoost regime probabilities and define regime confidence $c(t) = 0.5 + 0.5\max(p_{\mathrm{on}},\,p_{\mathrm{neutral}},\,p_{\mathrm{off}})$. The per-book multipliers are:
\begin{align*}
  m_{\text{mispricing}}  &= c\,(1 + 0.25\,p_{\mathrm{on}} - 0.10\,p_{\mathrm{off}}) \\
  m_{\text{core\_rank}}  &= c\,(1 + 0.35\,p_{\mathrm{off}}) \\
  m_{\text{priced\_tilt}}&= c\,(0.55 + 0.35\,p_{\mathrm{on}})
\end{align*}
and $\tilde{u}_b \leftarrow \tilde{u}_b \times m_b$.

\textbf{Step 4 — Priced-tilt cap.}
\begin{equation*}
  \tilde{u}_{\text{priced\_tilt}} \;\leftarrow\; \min\!\Bigl(\tilde{u}_{\text{priced\_tilt}},\;0.18\sum_b \tilde{u}_b\Bigr)
\end{equation*}

\textbf{Step 5 — Normalise.}
\begin{equation*}
  w_b(t) = \frac{\max(\tilde{u}_b(t),\,0)}{\displaystyle\sum_{b'} \max(\tilde{u}_{b'}(t),\,0)}
\end{equation*}

The Sharpe Ensemble's high realised MispricingM weight ($80.8\%$) is therefore not hard-coded: it emerges because the rolling scorer consistently finds MispricingM's 26-week Sharpe and drawdown profile superior, and because MispricingM's regime multiplier is the least penalised across regimes. The Defensive Ensemble's low allocation to MispricingM relative to Sharpe is a consequence of its lower base prior ($65\%$ vs.\ $92\%$), which anchors the allocator closer to a balanced position.
...
\begin{callout} [title=Why MispricingM dominates and Priced Tilt shrinks]
The allocation outcome reflects a fundamental distinction between two ways a factor can ``work.''

MispricingM's components (RMOM1w, RMOM2w, SMBC, NetRel) were selected because their \emph{joint return distribution} almost-stochastically dominates Bitcoin on a week-by-week basis. They earn their returns through repeated short-horizon mispricing corrections---each rebalance is an independent opportunity to extract the edge. This is precisely what a weekly long-short strategy needs: a signal whose value is realized within the rebalancing interval. These are also all price-based signals: momentum, size, and network-relative rotation are computed entirely from observed returns, with no exogenous data needed.

Priced Tilt factors (TVLC, CRASH8, BETA26, SKEW52, NEWC) are different in kind. They are \emph{compensated risk exposures}: the GX test confirms that assets loading on these characteristics earn systematically different expected returns, but that premium accumulates over months and years as investors are compensated for bearing persistent behavioral or fundamental risk. The week-to-week IC of these factors is weak precisely because the risk premium is structural, not episodic. A buy-and-hold investor tilting a portfolio toward high-CRASH8 or low-TVLC assets would capture the premium naturally; a weekly-rebalancing long-short book cannot, because it closes and reopens positions before the horizon over which the premium is earned.

\textbf{TVLC is the cleanest illustration of this for on-chain data specifically.} A protocol's TVL is observable in real time through the Artemis Terminal feed, but the \emph{repricing} that connects TVL to expected returns moves slowly: investors compare protocols, reallocate capital between them, and earn or pay the engagement premium over months as positioning shifts. The week-to-week noise in returns swamps the daily updates in TVL, so an IC test sees nothing useful, while a multi-year GX priced-risk test detects a $|t|=4.71$ effect. The same logic applies in principle to any on-chain engagement family---protocol fees, DEX volume, active-address counts, stablecoin inflows: these are structural quality signals that compound into expected returns through investor reallocation, not through week-to-week mispricing. Crypto's unique data advantage---a fully observable protocol balance sheet, updated continuously---is therefore most powerful when it is used to identify priced systematic risk rather than to forecast next-week returns. They belong on the priced-risk side of the factor router, not on the IC side.

In short: MispricingM earns high allocation because price-based weekly signals are exactly what a weekly rebalancing strategy can extract. Priced Tilt earns low allocation because its on-chain (TVLC) and behavioral components carry structural risk premia that accumulate over months rather than weeks. The rolling-performance scorer learns this distinction from the data; the appropriate response is not to discard Priced Tilt but to size it small and acknowledge that a longer-horizon allocator would be the right vehicle for capturing these slow premia.
\end{callout}
...
\toprule
\rowcolor{tablehead}
\textbf{Strategy} & \textbf{Sharpe} & \textbf{Ann.\ return} & \textbf{Vol} & \textbf{Max DD} & \textbf{Hit rate} & \textbf{Turnover} & \textbf{Weeks} \\
\midrule
SE  & $+0.84$ & $+29.9\%$ & $35.8\%$ & $-24.7\%$ & 51\% & ${\sim}40\%$ & 79 \\
BE  & $+0.68$ & $+18.8\%$ & $27.6\%$ & $-18.3\%$ & 53\% & ${\sim}37\%$ & 79 \\
DE  & $+0.75$ & $+21.6\%$ & $29.0\%$ & $-19.7\%$ & 53\% & ${\sim}36\%$ & 79 \\
MM  & $+0.85$ & $+37.2\%$ & $44.1\%$ & $-29.9\%$ & 52\% & ${\sim}42\%$ & 79 \\
CR  & $+0.11$ & $+2.0\%$  & $18.1\%$ & $-18.8\%$ & 47\% & ${\sim}28\%$ & 79 \\
PT  & $-0.43$ & $-13.7\%$ & $31.8\%$ & $-32.6\%$ & 39\% & ${\sim}32\%$ & 79 \\
...
\end{center}
\smallskip
{\small Table\,10.\;Out-of-sample performance (79\;weeks). Turnover is weekly one-sided average; benchmarks are passive and excluded.}
...
\subsection{Sub-book Performance Across Windows}
...
\toprule
\rowcolor{tablehead}
\textbf{Sub-book} & \textbf{Window} & \textbf{Sharpe} & \textbf{Ann.\ return} & \textbf{Vol} & \textbf{Max DD} \\
\midrule
\multirow{3}{*}{MispricingM}
  & Full (374\;wks) & $+1.24$ & $+76.5\%$ & $61.9\%$ & $-46.6\%$ \\
  & IS  (295\;wks) & $+1.32$ & $+87.0\%$ & $65.8\%$ & $-46.6\%$ \\
  & OOS (79\;wks)  & $+0.85$ & $+37.2\%$ & $44.1\%$ & $-29.9\%$ \\
\midrule
\multirow{3}{*}{Core Rank}
  & Full (374\;wks) & $+0.53$ & $+18.5\%$ & $35.1\%$ & $-49.3\%$ \\
  & IS  (295\;wks) & $+0.60$ & $+23.0\%$ & $38.3\%$ & $-49.3\%$ \\
  & OOS (79\;wks)  & $+0.11$ & $+2.0\%$  & $18.1\%$ & $-18.8\%$ \\
\midrule
\multirow{3}{*}{Priced Tilt}
  & Full (374\;wks) & $+0.30$ & $+11.1\%$ & $36.9\%$ & $-49.1\%$ \\
...
\end{center}
\smallskip
{\small Sub-book IS\;/\;OOS performance. IS window: 2021-05-10 \textendash{} 2024-11-11. OOS window: 2024-11-25 \textendash{} 2026-05-25.}
...
\toprule
\rowcolor{tablehead}
\textbf{Sub-book} & \textbf{IS Sharpe} & \textbf{OOS Sharpe} & \textbf{Decay} \\
\midrule
...
\end{center}
\smallskip
{\small Sub-book IS-to-OOS Sharpe decay.}
...
\textbf{Failure modes.}\;
The strategy would likely fail or weaken if MispricingM stopped working, small-cap liquidity disappeared, trading costs rose, factor crowding compressed returns, the regime-sizing model became unreliable in a new market environment, or on-chain data quality degraded---for example through Artemis Terminal coverage gaps for protocol fees or TVL, or through protocol-level events (oracle manipulation, double-counting across bridged TVL) that distort the underlying ratios that drive FunC and TVLC.
