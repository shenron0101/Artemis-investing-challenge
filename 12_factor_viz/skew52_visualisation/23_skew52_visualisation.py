"""Stage 10 — SKEW52 (Lottery Memory / Right-Tail Salience) Visualisation.

SKEW52 = high_minus_low 52-week realized skewness: long coins with the most
right-skewed weekly return histories, short the least skewed.

Characteristic: rolling(52).skew() on weekly returns.
Direction: +1 (long high-skew = salient upside history, short low-skew).

Grade: Priced risk — joint GX λ = +96.9%/yr, t = +3.44.
Investors remember and price coins that have had dramatic right-tail jumps.

Outputs → artifacts/figures/skew52_*.html/.png  +  SKEW52_VIZ_REPORT.md
"""
...
PREFIX = "skew52"
CHAR_COL = "skew_52w"
DIRECTION = +1   # long high skew, short low skew
FRAC = 0.30
MIN_NAMES = 10
T_LABELS = ["Low-Skew", "Mid-Skew", "High-Skew"]
SPREAD_LABEL = "SKEW52 (High−Low)"
...
def build_characteristics(panel, trade_symbols):
    ...
    skew52 = ret_wide.rolling(52).skew()
    ...
    for df, nm in [(fwd, "fwd_ret_1w"), (skew52, CHAR_COL)]:
...
def chart_cumulative_return(ret, is_lo, is_hi, oos_lo, oos_hi):
    ...
    fig.update_layout(template=PLOTLY_TEMPLATE, height=500,
                      title=f"{FACTOR_NAME} Cumulative Return (High-Skew − Low-Skew)",
...
def chart_tercile_returns(chars, is_lo, is_hi, oos_lo, oos_hi):
    ...
    fig.update_layout(template=PLOTLY_TEMPLATE, height=500, barmode="group",
                      title=f"{FACTOR_NAME} — Tercile Returns by 52-Week Skewness",
                      xaxis_title="Skewness Group", yaxis_title="Annualised Return (%)",
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig, spread
...
def chart_cumulative_tercile(chars, is_lo, is_hi, oos_lo, oos_hi):
    ...
    fig.update_layout(template=PLOTLY_TEMPLATE, height=550,
                      title="Cumulative Return by 52-Week Skewness Tercile",
                      xaxis_title="Week", yaxis_title="Growth of $1",
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig


def chart_cross_section_skew_over_time(chars, is_lo, is_hi, oos_lo, oos_hi):
    """Chart 9: Cross-sectional distribution of realized skewness over time.
    Shows when the market had the most 'lottery' coins (high dispersion in skewness)
    and whether those periods correspond to stronger SKEW52 performance."""
    chars = chars.dropna(subset=[CHAR_COL]).copy()
    weekly_p10 = chars.groupby("week")[CHAR_COL].quantile(0.10)
    weekly_p90 = chars.groupby("week")[CHAR_COL].quantile(0.90)
    weekly_median = chars.groupby("week")[CHAR_COL].median()

    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08,
                        subplot_titles=("Cross-Sectional Skewness Distribution (p10, median, p90)",
                                        "Skewness Spread (p90 − p10) — Factor Signal Strength"))
    fig.add_trace(go.Scatter(x=weekly_p90.index, y=weekly_p90.values, name="p90 (lottery-rich)",
                             line=dict(color=COLORS["SIG_POS"], width=1.5)), row=1, col=1)
    fig.add_trace(go.Scatter(x=weekly_median.index, y=weekly_median.values, name="Median",
                             line=dict(color=COLORS["FULL"], width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=weekly_p10.index, y=weekly_p10.values, name="p10 (low-skew)",
                             line=dict(color=COLORS["SIG_NEG"], width=1.5)), row=1, col=1)
    fig.add_hline(y=0, line_dash="dash", line_color="#666", row=1, col=1)
    spread = (weekly_p90 - weekly_p10).rolling(4).mean()
    fig.add_trace(go.Scatter(x=spread.index, y=spread.values, name="Spread (p90−p10)",
                             fill="tozeroy", line=dict(color=COLORS["OOS"], width=2)), row=2, col=1)
    _add_vline(fig, is_hi, "IS / OOS"); _add_vrect(fig, is_lo, is_hi, COLORS["IS"]); _add_vrect(fig, oos_lo, oos_hi, COLORS["OOS"])
    fig.update_layout(template=PLOTLY_TEMPLATE, height=700,
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    fig.update_yaxes(title_text="Realized Skewness", row=1, col=1)
    fig.update_yaxes(title_text="Skewness Spread", row=2, col=1)
    return fig
...
def write_report(is_d, oos_d, adf_stat, adf_p, jb_is, jb_is_p, jb_oos, jb_oos_p):
    md = f"""# SKEW52 (Lottery Memory / Right-Tail Salience) — Analysis Report
...
Joint GX-full result: **λ = +{GX_LAMBDA_ANN:.1f}%/yr, t = +{GX_T:.2f}**,
95% CI [{GX_CI_LO:.1f}%, {GX_CI_HI:.1f}%]. The cross-section prices the skewness
exposure even after all Stage-09 controls and the other behavioral factors.

The L/S return is noisy: sharpe = {fmt_val(is_d.get('sharpe', np.nan))} IS, {fmt_val(oos_d.get('sharpe', np.nan))} OOS.
...
**4. Correlation with RC (market) = {CORR_09:.3f}.**
...
![Skew distribution over time](artifacts/figures/{PREFIX}_09_cross_section_skew.png)
...
"""
    (STAGE / "SKEW52_VIZ_REPORT.md").write_text(md)
    print("  wrote SKEW52_VIZ_REPORT.md")
...
def main():
    ...
    factor_ret = beh.set_index("week")["SKEW52_high_minus_low"].dropna()
    ...
    fig9 = chart_cross_section_skew_over_time(chars, is_lo, is_hi, oos_lo, oos_hi)
